...
export class EOwnTimeout {
	...
	private callback: (...args: any[]) => void | Promise<unknown>;
	...
	private execute() {
		...
		const callbackCallResult = this.callback(...this.args);
		if (callbackCallResult instanceof Promise) {
			callbackCallResult.catch((error: unknown) => {
				(console as Console).error("Unhandled exception inside EOwnTimeout", error, ...this.args);
			});
		}
	}
